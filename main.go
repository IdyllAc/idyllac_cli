...
func main() {
	...
	goth.UseProviders(
		facebook.New(
			...
			"https://anypay.cards/auth/facebook/callback",
		),
		google.New(
			os.Getenv("GOOGLE_KEY"),
			os.Getenv("GOOGLE_SECRET"),
			"https://anypay.cards/auth/google/callback",
			"email", "profile",
		),
		github.New(
			os.Getenv("GITHUB_KEY"),
			os.Getenv("GITHUB_SECRET"),
			"https://anypay.cards/auth/github/callback",
		),
	)
	
	// db, err = sql.Open("sqlite", "./subscribe/DB_subscribers.db")
	   db, err = sql.Open("mysql", "./subscribe/authenticate_login")
